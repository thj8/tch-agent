...
```bash
tch-agent runtime build-image
# → 自动 build solver 镜像（首次较慢，之后秒级）

tch-agent runtime ping
# → ✓ Docker daemon connected

tch-agent runtime has-image
# → ✓ Image tch-agent:latest exists
```
...
```dockerfile
LABEL ai.tch-agent.dockerfile-sha256=abc123...
```
...
在 `apps/cli/src/main.ts` 加 `runtime` 命令组：
...
```
Image tch-agent:latest up to date (hash=abc12345)
✓ Done
```
...
```bash
docker images | grep tch-agent
# tch-agent   latest   abc123...   5 minutes ago   500MB
```

### 4.6 手动测试镜像

```bash
docker run -it --rm tch-agent:latest bash
# 进入容器内的 bash
