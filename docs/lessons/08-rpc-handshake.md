...
```bash
tinyfat runtime launch --prompt SOLVER "ls /tmp"
# → 容器启动 + init 握手成功
...
export type RpcCommand =
    ...
    // Host bridge：宿主 → 容器，把 bridge 请求的结果推回去（课时 9 引入）
...
async function handleCommand(session: AgentSession, cmd: RpcCommand): Promise<RpcResponse> {
    ...
    switch (cmd.type) {
        ...
        case "host_bridge_response": {
            // host bridge（课时 9）：宿主把之前 host_bridge_request 的结果推回来，
            // resolve 掉容器内挂起的 Promise。
...
先把 `runSolverRpc` 加到顶部 `@my/core` 的 import：

```typescript
import { ConfigManager, runSolverCli, runSolverRpc, RuntimeManager } from "@my/core"
```

（`runSolverCli` 来自 lesson 5、`RuntimeManager` 来自 lesson 6，这次只是把 `runSolverRpc` 一并加上 —— `@my/core` 是内部包，统一顶部 import。）
...
骨架事件看完了，下面用一个**真实跑通**的例子，把「宿主命令 → 容器内 LLM → 事件流回宿主」整条链路逐行拆开，看清每个 `[xxx]` 背后到底是什么事件。

#### 命令

```bash
bun run apps/cli/src/main.ts runtime launch --prompt SOLVER "写一个python语言版本hello world，并运行"
```

> 出现 `✓ Launched solver ebb2fe4c` 就说明 **init 握手成功**了 —— `launch()` 内部 `await` 了 `initReady`，30 秒内收到容器回的 `{command:"init",success:true}` 才会走到这行。

#### 终端实际输出

```
✓ Launched solver ebb2fe4c (container: tch-solver-ebb2fe4c)
Press Ctrl+C to stop...

[ebb2fe4c] [agent_start]
[ebb2fe4c] [turn_start]
[ebb2fe4c] [message_start]
[ebb2fe4c] [message_start]
[ebb2fe4c] [assistant] 我来创建一个 Python Hello World 程序并运行它。
[ebb2fe4c] [tool_call] write({"path":"hello.py","content":"print(\"Hello, World!\")\n"})
[ebb2fe4c] [tool_result] Successfully wrote 23 bytes to hello.py
[ebb2fe4c] [message_start]
[ebb2fe4c] [turn_end]
[ebb2fe4c] [turn_start]
[ebb2fe4c] [message_start]
[ebb2fe4c] [tool_call] bash({"command":"python3 hello.py"})
[ebb2fe4c] [tool_result] Hello, World!

[ebb2fe4c] [message_start]
[ebb2fe4c] [turn_end]
[ebb2fe4c] [turn_start]
[ebb2fe4c] [message_start]
[ebb2fe4c] [assistant] 完成！
- **文件**：`hello.py`，内容为 `print("Hello, World!")`
- **运行结果**：成功输出 `Hello, World!`
[ebb2fe4c] [turn_end]
[ebb2fe4c] [agent_end] stopReason=stop
```

#### 打印机在哪：`summarizeEvent`

每行 `[ebb2fe4c] <内容>` 都来自 `apps/cli/src/main.ts` 里 `runtime launch` 注册的事件回调 —— 容器 stdout 每吐一行 JSONL（一个 `AgentSessionEvent`），宿主 `emit` 给它，它翻译成人话打印：
...
`summarizeEvent`（`apps/cli/src/main.ts`）的翻译规则：

| 容器发来的事件 | 终端打印 | 含义 |
|---|---|---|
| `agent_start` | `[agent_start]` | agent 循环开始（一次 prompt 一对 start/end） |
| `turn_start` | `[turn_start]` | 一个 turn 开始（**一次 LLM 调用 = 一个 turn**） |
| `message_start`（role=user） | `[message_start]` | SDK 把用户 prompt 包成一条 user 消息 |
| `message_start`（role=assistant） | `[message_start]` | LLM 回复开始（content 此时还是空） |
| `message_end`（assistant，有文本） | `[assistant] <文本>` | LLM 回复落定，抽出文本部分 |
| `tool_execution_start` | `[tool_call] <名>(<参数>)` | SDK 准备执行工具 |
| `tool_execution_end` | `[tool_result] <预览>` | 工具执行完，结果前 500 字 |
| `message_start`（role=toolResult） | `[message_start]` | 工具结果包成一条 toolResult 消息 |
| `turn_end` | `[turn_end]` | turn 结束（带上本 turn 的工具结果） |
| `agent_end` | `[agent_end] stopReason=<r>` | agent 结束，取最后一条 assistant 的 stopReason |

> 💡 **为什么有些消息只看到 `[message_start]`，没有对应的 `[message_end]`？**
> `summarizeEvent` 对 `message_end` 只在 `role==="assistant"` **且有文本**时才打印成 `[assistant]`，其余（user / toolResult / 空内容）返回 `null` 被丢弃；而 `message_start` 没有专门分支，走 `default` 原样打印成 `[message_start]`。所以 user / toolResult 的 start 有、end 没有。

#### 按 turn 拆：三个 turn 各干了什么

agent 循环 = 一串 turn，**每个 turn = 一次 LLM 调用 + 它触发的工具**。这次跑了 3 个 turn：

**Turn 1 —— 写文件**
- 用户 prompt 进来 → LLM 决定先建文件
- `[assistant] 我来创建…` + `[tool_call] write(hello.py)`：LLM 这条回复**同时**带了文本和 `write` 工具调用（`stopReason=toolUse`）
- `[tool_result] Successfully wrote 23 bytes`：`write` 在容器 cwd 落盘成功

**Turn 2 —— 运行文件**
- 上一轮工具结果喂回 LLM → 它决定运行
- `[tool_call] bash(python3 hello.py)` → `[tool_result] Hello, World!`：真在容器里跑了，stdout 是 `Hello, World!`

**Turn 3 —— 总结收尾**
- 运行结果喂回 LLM → 它认为任务完成
- `[assistant] 完成！…`：纯文本总结，**没有工具调用** → `stopReason=stop` → 触发 `[agent_end] stopReason=stop`

#### 产物落盘

工具在容器 cwd（`/root/workspace`，挂载到宿主 `~/.tinyfat/solvers/<id>/workspace/`）里操作，宿主直接能看到：

```bash
$ cat ~/.tinyfat/solvers/ebb2fe4c/workspace/hello.py
print("Hello, World!")
```

#### stopReason 速查

| stopReason | 含义 | agent 接下来 |
|---|---|---|
| `toolUse` | LLM 想调工具 | 继续下一个 turn（把工具结果喂回去） |
| `stop` / `end_turn` | LLM 认为做完了 | 结束（`agent_end`） |
| `error` | 这次 LLM 调用 API 失败 | 结束（详情见第五步「问题 7」） |
...
docker run -i --rm \
  ...
  tinyfat:latest \
  /opt/tinyfat/tinyfat solver rpc
```
...
**原因**：容器内看不到 `~/.tinyfat/config/prompts/SOLVER.md`。

**解决**：检查 `-v ~/.tinyfat:/root/.tinyfat` 挂载是否正确。

### 问题 6：API Key 找不到

**原因**：容器内看不到 `~/.tinyfat/config/auth.json`，或 auth.json 里没存 OpenAI 的 key。
...
### 问题 7：`[agent_end] stopReason=error`，但看不到任何错误详情

**现象**：agent 跑了一半（文件写了、工具也调了），最后只剩一行 `[agent_end] stopReason=error`，没有任何错误信息。

**原因**：底层这次 LLM 调用 API 失败了（走第三方代理时常见，例如智谱 GLM 返回 `code 1234 网络错误，请稍后重试`）。失败的那条 assistant 消息 `content` 是空数组、带个 `errorMessage` 字段；但 `summarizeEvent` 只打印 assistant 的**文本**和 `stopReason`，**不看 `errorMessage`**，所以错误被静默吞掉。

**定位**：翻容器日志，里面每条事件都是完整 JSONL，`errorMessage` 就藏在那儿：

```bash
docker logs tch-solver-<id>
# 找 errorMessage 字段，例如：
# "errorMessage":"{\"error\":{\"type\":\"api_error\",\"code\":\"1234\",\"message\":\"[1234][网络错误，错误id：…，请稍后重试]\"}}"
```

**处理**：

- `code 1234 网络错误` 是代理侧瞬时抖动，**直接重跑**同一个 prompt 通常就通了（第一次调用往往是成功的，失败的多半是「拿到工具结果后继续」那一跳）。
- 想让错误不再静默：在 `summarizeEvent` 的 `message_end` 分支里，当 `stopReason==="error"` 时把 `errorMessage` 也打出来。
